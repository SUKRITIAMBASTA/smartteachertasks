...
// Canonical slot strings — must match exactly what seed.cjs writes to DB
const ALL_SLOTS = [
...
// Normalise slot string for comparison (strip all whitespace, lowercase)
...
export default function ScheduleManagerPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (Array.isArray(deptRes)) {
        setDepts(deptRes);
        const uniqueSchools = Array.from(new Set(deptRes.map((d: any) => d.name))) as string[];
  ...
  useEffect(() => { fetchRoutines(); }, [scheduleType, selectedBranch, selectedSem]);
  ...
  const handleApproveAll = async () => {
    if (!confirm('Institutional Bulk Publication: This will publish ALL pending schedules across ALL departments. Proceed?')) return;
    setLoadingSchedule(true);
    try {
      const res = await fetch('/api/schedule/approve', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({}) // Empty body for bulk approval
      });
      const data = await res.json();
      if (data.success) {
        toast.success(`Successfully published ${data.count} schedules.`);
        fetchRoutines();
      }
    } catch { setError('Bulk approval protocol failure.'); }
    finally { setLoadingSchedule(false); }
  };
  ...
  const isPending = useMemo(() => schedules.some(s => s.status === 'pending_approval'), [schedules]);
  const branchName = useMemo(() => depts.find(d => d._id === selectedBranch)?.branch || 'N/A', [depts, selectedBranch]);
  const currentRoomNo = useMemo(() => schedules[0]?.roomId?.roomNo || '—', [schedules]);

  // Build grid — normalise slot strings for comparison
  const routineGrid = useMemo(() => {
    ...
    schedules.forEach(s => {
      ...
      const matchingSlot = currentConfig.slots.find(
        slot => normaliseSlot(slot) === normaliseSlot(s.timeSlot)
      );
      if (grid[dayName] && matchingSlot) {
        grid[dayName][matchingSlot] = s;
      }
    });
    return grid;
    ...
    <div className={`space-y-6 pb-12 transition-all duration-700 ${isFocused ? 'p-8 bg-slate-50 min-h-screen' : ''}`}>
        ...
        <div className="flex items-center gap-3">
          <button onClick={toggleFocus} className="p-2.5 bg-white border border-slate-200 rounded-xl text-slate-500 hover:text-blue-600 hover:border-blue-200 transition-all shadow-sm">
            ...
            <button
              ...
              className="px-6 py-3 bg-blue-600 text-white rounded-xl font-black text-[10px] tracking-[0.15em] shadow-lg hover:bg-blue-700 transition-all flex items-center gap-2 active:scale-95"
            >
              <ShieldCheck size={14} /> PUBLISH ALL PENDING
            </button>
        </div>
      </div>

      {error && (
        <div className="p-3 bg-rose-50 border border-rose-100 text-rose-600 rounded-xl text-sm font-bold">
          {error}
        </div>
      )}
      ...
      <div className={`grid grid-cols-1 ${isFocused ? '' : 'xl:grid-cols-4'} gap-6 ${isFocused ? 'max-w-[1400px] mx-auto' : ''}`}>
            ...
            <div className="glass-card p-5 border-slate-200 bg-white shadow-xl shadow-blue-500/5">
              ...
              <div className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <select
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl font-bold text-xs"
        ...
        <div className={`${isFocused ? 'w-full' : 'xl:col-span-3'} space-y-4`}>
            ...
            {schedules.length > 0 && isPending && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
                ...
                <button onClick={handleApprove}
                  className="px-5 py-2 bg-blue-600 text-white rounded-lg font-black text-[9px] tracking-widest shadow-lg hover:scale-105 active:scale-95 transition-all">
                  APPROVE & PUBLISH
                </button>
              </motion.div>
            )}
          ...
          {loadingSchedule ? (
            <div className="bg-white border-[3px] border-slate-900 rounded-sm p-20 flex items-center justify-center">
              <div className="flex flex-col items-center gap-3">
                <div className="w-10 h-10 border-4 border-blue-600 border-t-transparent rounded-full animate-spin" />
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Loading Schedule...</p>
              </div>
            </div>
          ) : (
                ...
                <div className="min-w-[1100px]">
                  <header className="text-center font-black border-b-[3px] border-slate-900">
                    ...
                    <div className={`${currentConfig.colors.bg} ${currentConfig.colors.text} py-1.5 text-[11px] tracking-widest font-black uppercase`}>
                      {currentConfig.header} — {branchName} · SEM {selectedSem} · Room {currentRoomNo}
                    </div>
                  </header>
                  ...
                  <div className="grid grid-cols-[80px_repeat(5,1fr)_40px_repeat(5,1fr)] bg-white">
                    ...
                    {DAYS.map(day => (
                      <React.Fragment key={day}>
                        <div className="border-r-[3px] border-b-[3px] border-slate-900 bg-slate-50 p-2 font-black text-[10px] flex items-center justify-center uppercase">
                          {day}
                        </div>
                        {currentConfig.slots.map((slot, idx) => {
                          ...
                          return (
                            <div
                              key={idx}
                              className={`border-r-[3px] border-b-[3px] border-slate-900 min-h-[85px] p-1.5 flex flex-col items-center justify-center text-center transition-all ${
                                item?.isLab ? 'bg-[#FFFF99]' : 'hover:bg-slate-50/80'
                              }`}
                            >
                              {item ? (
                                <div className="space-y-0.5 w-full">
                                  <div className="text-[8px] font-black leading-tight text-slate-800 uppercase tracking-tighter line-clamp-2">
                                    {item.subjectId?.name || '—'}
                                  </div>
                                  <div className="text-[7px] font-bold text-slate-400 italic truncate">
                                    {item.facultyId?.name || ''}
                                  </div>
                                  <div className="text-[7px] font-black text-blue-500 uppercase">
